...
final class ApiRequestor implements ApiRequestorInterface
{
    ...
    public function __construct(
        ...
        if (trim($this->apiToken) === '') {
            throw new AuthenticationException('API token must not be empty.');
        }
    ...
    public function request(
        ...
        try {
            $request = $this->createRequest($method, $uri, $query, $json);
     ...
     * @param null|array<string, mixed>  $json
     */
    private function createRequest(string $method, string $uri, array $query, ?array $json): RequestInterface {
